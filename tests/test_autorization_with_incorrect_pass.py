...
@allure.feature('Авторизация юзера на сайте телеканала СТС с некорректным паролем')
@allure.title('Страница авторизации юзера с некорректным Password')
def test_autorization_with_incorrect_password(browser):
    ...
    with step("Заполняет данные полей для авторизации с некорректным Email"):
        authorization_page.fill_data_in_authorization_page_with_incorrect_password()
